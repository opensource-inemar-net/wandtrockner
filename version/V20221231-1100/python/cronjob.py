...
def cronjob():
    ...
    #Get standort and current mode of the device
    ...
    if data == None:
        ...
        if not os.path.isfile("../../../daten/" + sanitize_filepath(standort) +"-daten.csv"):
            ...
            try:
                ...
                print("Error while trying to create new csv file")
        else:
        ...
        if modem != None:
            file.write(str(modem.imsi) + "," + str(modem.signalStrength) + "," + str(modem.write('AT+CPSI?')))
        else:
...
""" TODO:
Begrenzung der Anzahl an Personen, an die SMS geschickt werden können?
"""
